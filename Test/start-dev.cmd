...
if %ERRORLEVEL%==0 (
    echo Backend gia attivo: http://localhost:5001
) else (
    echo Avvio backend su http://localhost:5001 ...
    start "FilmAPI Backend" cmd /k "cd /d "%BACKEND_DIR%" && dotnet run --urls http://localhost:5001"
)

tasklist /FI "IMAGENAME eq FilmFrontend.exe" | find /I "FilmFrontend.exe" >nul
if %ERRORLEVEL%==0 (
    echo Frontend gia attivo: http://localhost:5285
) else (
    echo Avvio frontend su http://localhost:5285 ...
    start "FilmFrontend" cmd /k "cd /d "%FRONTEND_DIR%" && dotnet run --urls http://localhost:5285"
)
